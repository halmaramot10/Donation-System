...
@WebServlet(name = "Update_Food", urlPatterns = {"/Update_Food"})
public class Update_Food extends HttpServlet {
    ...
    Statement st;
    ResultSet rs;
    String desc,type,qty,exp,stat,donto,rec,don;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            exp = request.getParameter("expire");
            don = request.getParameter("released");
            
            session = request.getSession();
            con = DB.getConnection();
            st = con.createStatement();
            String sql = "UPDATE foods_donate SET "
                           + "item_desc = '"+desc+"'"
                           + ",item_type = '"+type+ "'"
                           + ",quantity = '"+qty+"'"                           
                           + ",status = '"+stat+"'"
            ...
            if(status>0)
                {
                    ...
                    response.sendRedirect("./food_edit_form.jsp?id="+id);
